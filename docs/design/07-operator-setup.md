...
3. If `unlock_code` → row in `mission_unlock_codes` (clearance code). The same string may appear on multiple missions so one **Request Access** lists several entries.  
